...
interface VectorpeaProps {
  filePath?: string;
}

export default function Vectorpea({ filePath }: VectorpeaProps) {
        ...
        save: `
          (function(){
            ...
            if(src.indexOf(".") != -1) {
              ...
              if(ext == "png" || ext == "jpg" || ext == "jpeg" || ext == "svg" || ext == "psd") fmt = (ext == "jpeg") ? "jpg" : ext;
            }
            app.echoToOE(src);
  ...
  const vectorpeaUrl = `https://www.vectorpea.com#${encodeURIComponent(JSON.stringify(config))}`;

  // Unified save logic that "ties" the two events together
  const processSave = async (path: string, data: ArrayBuffer) => {
    let targetPath = path;

    // If it's a new file created in Vectorpea, show "Save As" dialog
    if (path.startsWith("local,")) {
      const suggestedName = path.split(",").pop() || "image.svg";
      const ext = suggestedName.split(".").pop()?.toLowerCase() || "svg";

      const chosenPath = await showSaveFileDialog({
        initialPath: `/home/Pictures/${suggestedName}`,
        // Add a filter based on what Vectorpea told us the file is
        fileFilter: (node) => {
    ...
    try {
      ...
      // If we just saved a new file, tell Vectorpea its new home
      if (path.startsWith("local,")) {
      ...
      console.error("Vectorpea: Failed to save file to VFS", err);
      ...
      if (e.data === "done") {
        ...
        if (filePath && fileSentRef.current !== filePath) {
          try {
            ...
            console.error("Vectorpea: Failed to load file from VFS", err);
  ...
  return (
    <div className="w-full h-full bg-[#1c1c1c] relative flex items-center justify-center">
      ...
      <iframe ref={iframeRef} src={vectorpeaUrl} className="w-full h-full border-none" title="Vectorpea" />
    </div>
  );
}
